...
from app.keyboards.inline.pvp import selects_card_pvp
...
async def _(message:Message,session:AsyncSession):
    ...
    logger.info("Пользователь {id} смотрит колоду: {coloda}", id = user.id,
                coloda = user.battle_inventory.cards)
    ...
    await message.answer(MText.get("duels").format(cards=cards_text), reply_markup=await selects_card_pvp())
