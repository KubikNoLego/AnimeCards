...
from app.utils.constants import PLAYERS_IN_TOP
from app.utils.top_utils import (top_players_by_balance_formatter, 
                                top_players_by_wins_formatter)
...
async def _(message: Message, session: AsyncSession):
    ...
    if user:
        top_players_balance = await db.user.get_top_players_by_season_balance(
            PLAYERS_IN_TOP
        )
        text_balance = await top_players_by_balance_formatter(
                                                        top_players_balance,
                                                        user.id, True)
